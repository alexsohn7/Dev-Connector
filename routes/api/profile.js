...
// @desc  Get profiles for all the users
// @access Public
  ...
  Profile.find()
    ...
    .then(profiles => {
      if (!profiles) {
        ...
        return res.status(404).res.json(errros);
      }
      res.json(profiles);
    })
    .catch(err => {
...
router.post(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      ...
      // Adding the experience form data to the experience array
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        ...
        // Splice the index out of the experience array
